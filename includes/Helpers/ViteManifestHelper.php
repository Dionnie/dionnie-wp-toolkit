...
class ViteManifestHelper
{
    ...
    {
        return file_exists(rtrim(DIONNIE_WP_TOOLKIT_PATH, '/') . '/public/hot');
    }
    ...
    {
        $manifestPath = rtrim(DIONNIE_WP_TOOLKIT_PATH, '/') . '/public/build/manifest.json';
        ...
        $assetsUrl = rtrim(DIONNIE_WP_TOOLKIT_URL, '/') . '/public/build/';
        ...
        foreach ($entries as $entryKey => $config) {
            ...
            if (preg_match('/\.(js|jsx|ts|tsx)$/', $fileKey)) {
                wp_enqueue_script($handle, $fileUrl, $deps, DIONNIE_WP_TOOLKIT_VERSION, $inFooter);

                if (!empty($asset['css']) && is_array($asset['css'])) {
                    foreach ($asset['css'] as $index => $cssFile) {
                        $cssHandle = $handle . '-css-' . (string)$index;
                        wp_enqueue_style($cssHandle, $assetsUrl . ltrim($cssFile, '/'), [], DIONNIE_WP_TOOLKIT_VERSION);
                    }
                }
            }
        }
    ...
    private function generateHandle(string $entryKey): string
    {
        ...
        return DIONNIE_WP_TOOLKIT_SLUG . '-' . sanitize_title($cleanName);
    }
}
